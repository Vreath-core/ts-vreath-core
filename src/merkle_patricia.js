...
const en_key = (key) => {
    return rlp_1.default.encode(key).toString();
};
const de_key = (key) => {
    return rlp_1.default.decode(key).toString();
};
const en_value = (value) => {
    return rlp_1.default.encode(JSON.stringify(value)).toString();
};
const de_value = (value) => {
    return JSON.parse(rlp_1.default.decode(value).toString());
};
class Trie {
    constructor(db, root = "") {
        if (root == "")
