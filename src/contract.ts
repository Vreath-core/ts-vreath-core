...
export const native_prove = (bases:string[],base_state:T.State[],input_data:string[],):T.State[]=>{
    ...
    switch(type){
      case "0x0":
        ...
        const sum = amounts.reduce((s,a)=>bigInt(a).add(s),bigInt(0));
        const fee = bigInt(remiter_state.data[0]||"0x0");
        const gas = bigInt(remiter_state.data[1]||"0x0");
        const income = bigInt(remiter_state.data[2]||"0x0");
        if(bigInt(remiter_state.amount).subtract(sum).subtract(fee).subtract(gas).subtract(income).lesser(0)||receivers.length!=amounts.length) return base_state;

        const remited = base_state.map(s=>{
          if(s.token!=native||s.owner!=remiter) return s;
          const income = bigInt(s.data[2]||"0x0");
          return _.new_obj(
            s,
            (s)=>{
              s.nonce = '0x'+bigInt(s.nonce).add(1).toString(16);
              s.amount = '0x'+bigInt(s.amount).subtract(income).subtract(sum).toString(16);
              return s;
            }
          )
        });
        const recieved = remited.map(s=>{
          ...
          return _.new_obj(
            ...
            s=>{
              s.nonce = '0x'+bigInt(s.nonce).add(1).toString(16);
              s.amount ='0x'+bigInt(s.amount).subtract(income).subtract(amounts[index]).toString(16);
              return s;
            }
          )
        });
...
export const native_verify = (bases:string[],base_state:T.State[],input_data:string[],output_state:T.State[])=>{
    ...
    switch(type){
        case "0x0":
            ...
            const fee = bigInt(remiter_state.data[0]||"0x0");
            const gas = bigInt(remiter_state.data[1]||"0x0");
            if(bigInt(remiter_state.amount).subtract(sum).subtract(fee).subtract(gas).lesser(0)||receivers.length!=amounts.length) return false;

            const remited = base_state.some((s,i)=>{
                if(s.token!=native||s.owner!=remiter) return false;
                const income = bigInt(s.data[2]||"0x0");
                const output = output_state[i];
                return bigInt(output.nonce).subtract(s.nonce).notEquals(1) || s.owner!=output.owner || bigInt(s.amount).subtract(income).subtract(sum).notEquals(output.amount);
            });
            ...
            const recieved = base_state.map((s,i)=>{
                ...
                const income = bigInt(s.data[2]||"0x0");
                const output = output_state[i];
                return bigInt(output.nonce).subtract(s.nonce).notEquals(1) || s.owner!=output.owner || bigInt(s.amount).subtract(income).add(amounts[index]).notEquals(output.amount);
            });
    ...
    switch(type){
        case "0x0":
            ...
            const unit_verify = P.some(units,async (unit,i)=>{
                ...
                return !bigInt(hash).lesserOrEquals(constant.pow_target) || '0x'+unit_base_hash_parts[i+1]!=iden || unit_states[i].data.length!=0;
            });
            ...
            const unit_price_map:{[key:string]:BigInteger} = units.reduce((res:{[key:string]:BigInteger},unit)=>{
                ...
                if(res[hash]==null){
                res[hash] = bigInt(unit[4]);
                return res;
                }
                else{
                res[hash] = bigInt(res[hash]).add(unit[4]);
                return res;
                }
            },{});
            ...
            const unit_bought = base_state.map(s=>{
                ...
                const reduce = bigInt(last_height).subtract(pre_height);
                const amount = (()=>{
                    const computed = (bigInt(s.amount).add(unit_sum)).multiply(bigInt(constant.unit_rate).pow(reduce)).divide(bigInt(100).pow(reduce));
                    if(computed.lesser(1)) return bigInt("0x0");
            ...
            const native_input = native_base_hash_parts.map(key=>unit_price_map[key]||bigInt(0)).map(big=>'0x'+big.toString(16));
    ...
    switch(type){
        case "0x0":
            ...
            const unit_bought = base_state.some((s,i)=>{
                ...
                if(pre_flag==="0x0"||new_flag!="0x1") return true;
                const pre_height = s.data[1];
                const reduce = bigInt(last_height).subtract(pre_height);
                const amount = (()=>{
                    const computed = (bigInt(s.amount).add(unit_sum)).multiply(bigInt(constant.unit_rate).pow(reduce)).divide(bigInt(100).pow(reduce));
                ...
                return bigInt(output.nonce).subtract(s.nonce).notEquals(1) || s.owner!=output.owner || amount.notEquals(output.amount) || pre_flag==="0x0" || new_flag!="0x1" || output.data[1]!=last_height || bigInt(output.data[1]).lesserOrEquals(s.data[1]);
            });
            ...
            const unit_used = base_state.some((s,i)=>{
                ...
                const output = output_state[i];
                return bigInt(output.nonce).subtract(s.nonce).notEquals(1) || s.owner!=output.owner || s.data[0]!=null || output.data[0]!="0x0" || output.data[1]!=last_height || bigInt(output.data[1]).lesserOrEquals(s.data[1]);
            });
...
export const req_tx_change = (base_state:T.State[],requester:string,fee:string,gas:string)=>{
    const reqed = base_state.map(s=>{
        ...
        return _.new_obj(
            s,
            s=>{
                if(s.data[0]==null) s.data[0] = "0x0";
                else s.data[0] = '0x'+bigInt(s.data[0]).add(fee).toString(16);
                s.data[1] = '0x'+bigInt(gas).toString(16);
                return s;
            }
        )
    });
...
export const ref_tx_change = async (bases:string[],base_state:T.State[],requester:string,refresher:string,fee:string,gas:string,last_height:string)=>{
    const reqed = base_state.map(s=>{
        ...
        return _.new_obj(
            ...
            s=>{
                s.data[1] = "0x0";
                s.amount = '0x'+bigInt(s.amount).subtract(gas).toString(16);
                return s;
            }
        )
    });
    const refed = reqed.map(s=>{
        ...
        return _.new_obj(
            s,
            s=>{
                s.amount = '0x'+bigInt(s.amount).add(gas).toString(16);
    ...
    const gained = refed.map(s=>{
        ...
        return _.new_obj(
            s,
            s=>{
                s.amount = '0x'+bigInt(s.amount).add(income).toString(16);
                s.data[2] = "0x0";
                return s;
            }
        )
    ...
    const reduced = gained.map(s=>{
        if(s.token!=constant.unit||bases.indexOf(s.owner)===-1||s.data[0]!="0x1") return s;
        const pre_height = s.data[1];
        const reduce = bigInt(last_height).subtract(pre_height);
        const amount = (()=>{
            const computed = bigInt(s.amount).multiply(bigInt(constant.unit_rate).pow(reduce)).divide(bigInt(100).pow(reduce));
            if(computed.lesser(1)) return bigInt("0x0");
            else return computed;
        })();
        return _.new_obj(
...
export const key_block_change = (base_state:T.State[],validator_1:string,validator_2:string,fee:string,last_height:string)=>{
    const fee_1 = bigInt(fee).multiply(4).divide(10);
    const fee_2 = bigInt(fee).multiply(6).divide(10);
    const paid = base_state.map(s=>{
        if(s.token!=constant.native) return s;
        const fee = bigInt(s.data[0]||"0x0")
        if(fee.eq(0)) return s;
        return _.new_obj(
            s,
            s=>{
                s.amount = '0x'+bigInt(s.amount).subtract(fee).toString(16);
                s.data[0] = "0x0";
                return s;
            }
        )
    ...
    const gained = paid.map(s=>{
        ...
        return _.new_obj(
            s,
            s=>{
                s.amount = '0x'+bigInt(s.amount).add(gain).toString(16);
                s.data[2] = '0x'+bigInt(s.data[2]||"0x0").add(gain).toString(16);
                return s;
            }
        )
    });
