...
export const search_micro_block = async (block_db:DB,key_block:T.Block,last_height:string)=>{
    const key_public = get_info_from_block(key_block)[4];
    ...
    while(1){
        block = await block_db.read_obj(height);
        if(block==null) continue;
        public_key = get_info_from_block(block)[4];
...
export const verify_key_block = async (block:T.Block,block_db:DB,trie:Trie,state_db:DB,lock_db:DB,last_height:string)=>{
    ...
    const pre_key_block = await search_key_block(block_db,last_height);
    if(pre_key_block==null) return false;
...
export const verify_micro_block = async (block:T.Block,output_states:T.State[],block_db:DB,trie:Trie,state_db:DB,lock_db:DB,last_height:string)=>{
    ...
    else if((bigInt(height,16).mod(3).eq(0)&&txs.some((tx,i)=>tx.meta.kind===0&&_.array2hash(tx_tokens[i])!=unit_buying_tokens_hash||(bigInt(height,16).mod(3).notEquals(0)&&txs.some((tx,i)=>tx.meta.kind===0&&_.array2hash(tx_tokens[i])===unit_buying_tokens_hash))))){
...
export const create_key_block = async (private_key:string,block_db:DB,last_height:string,trie:Trie,state_db:DB,extra:string):Promise<T.Block>=>{
    ...
    const pre_key_block = await search_key_block(block_db,last_height) || empty_block();
    const pre_validator_pub = get_info_from_block(pre_key_block)[3];
    const pre_unit_validator = crypto_set.generate_address(constant.unit,pre_validator_pub);
    const pre_unit_validator_state = await data.read_from_trie(trie,state_db,pre_unit_validator,0,state_set.CreateState("00",constant.unit,pre_unit_validator,"00",["01","00"]));
